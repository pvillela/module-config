...
use crate::fs::{foo_ad_sfl_c, FooASflCfg, FooASflDeps, FooASflS, FooASflT};
...
//     };
//     let foo_ad_sfl_s = Arc::new(FooASflS { cfg, deps });
...
pub fn foo_ad_sfl_boot_lr(
    ...
    let s_ref_leak: &FooASflS = Box::leak(Box::new(FooASflS { cfg, deps })); // impl Deref
    let dyn_deref_ref_leak: &(dyn Deref<Target = FooASflS> + Send + Sync) =

...
pub fn foo_a_sfl_init_cached(app_cfg_src: fn() -> Arc<AppCfgInfo>) {
    // A stereotype should initialize its dependencies.
    bar_a_bf_init_refreshable(app_cfg_src);
    foo_a_sfl_adapt_cfg_src(app_cfg_src, RefreshMode::Cached, FooASflDeps { bar_a_bf });
}

// fn box_pin_async_fn<T>(f: fn() -> dyn Future<Output = T>) -> fn() -> Pin<Box<dyn Future<Output = T>> {

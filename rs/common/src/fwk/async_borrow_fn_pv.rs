...
pub trait AsyncBorrowFn1b1Pv<'a, A: ?Sized + 'a, Out>:
    Fn(&'a A) -> Self::Fut + Send + Sync
{
    type Fut: Future<Output = Out> + Send + Sync + 'a;
}

impl<'a, A, Out, F, Fut> AsyncBorrowFn1b1Pv<'a, A, Out> for F
where
    A: ?Sized + 'a,
...
pub trait AsyncBorrowFn2b2Pv<'a, A1, A2: ?Sized + 'a, Out>:
    Fn(A1, &'a A2) -> Self::Fut + Send + Sync
{
    type Fut: Future<Output = Out> + Send + Sync + 'a;
}

impl<'a, A1, A2, Out, F, Fut> AsyncBorrowFn2b2Pv<'a, A1, A2, Out> for F
where
    A2: ?Sized + 'a,
...
pub trait AsyncBorrowFn3b3Pv<'a, A1, A2, A3: ?Sized + 'a, Out>:
    Fn(A1, A2, &'a A3) -> Self::Fut + Send + Sync
{
    type Fut: Future<Output = Out> + Send + Sync + 'a;
}

impl<'a, A1, A2, A3, Out, F, Fut> AsyncBorrowFn3b3Pv<'a, A1, A2, A3, Out> for F
where
    A3: ?Sized + 'a,
...
pub fn partial_apply_async_borrow_fn_2b2_boxpin_pv<A1, A2, T>(
    f: impl for<'a> AsyncBorrowFn2b2Pv<'a, A1, A2, T>,
    a1: A1,
...
/// }
/// ```
pub fn partial_apply_async_borrow_fn_2b2_pv<A1, A2, F, T>(
    f: F,
    a1: A1,
) -> impl for<'a> AsyncBorrowFn1b1Pv<'a, A2, T>
where
    A1: Clone + Send + Sync + 'static,
    A2: ?Sized + 'static,
    F: for<'a> AsyncBorrowFn2b2Pv<'a, A1, A2, T> + 'static,
{
    ...
    where
        ...
        // v
        C: Fn(&A2) -> <F as AsyncBorrowFn2b2Pv<'_, A1, A2, T>>::Fut,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_all() {
        let f_part = partial_apply_async_borrow_fn_2b2_boxpin_pv(f0, 40);
        assert_eq!(42, f_part(&2).await);

        let f_part = partial_apply_async_borrow_fn_2b2_pv(f0, 40);
        assert_eq!(42, f_part(&2).await);

        // The commented-out lines below don't compile
        // let g = |x, y: &u32| f0(x, y);

        let f_part = partial_apply_async_borrow_fn_2b2_boxpin_pv(f1, 40);
        assert_eq!(43, f_part(&2).await);

        let f_part = partial_apply_async_borrow_fn_2b2_pv(f1, 40);
        assert_eq!(43, f_part(&2).await);

        let f_part = partial_apply_async_borrow_fn_2b2_boxpin_pv(f2, 40);
        assert_eq!(44, f_part(&2).await);

        let f_part = partial_apply_async_borrow_fn_2b2_pv(f2, 40);
        assert_eq!(44, f_part(&2).await);
    }
}
